...
- Estudiante: Maria Jose Linares Cortes
- Commit SHA evaluado: c35a35d690c86e8421ab1a89e305c485792b98fe
- Mi contribución concreta y enlace a archivo, commit o revisión: Escribí `tests/service-worker.spec.ts` (17 pruebas) y `tests/offline.spec.ts` (9 pruebas) validando la estructura de `public/sw.js` y el comportamiento del módulo `register-service-worker.ts`. También detecté y reporté una inconsistencia entre `docs/cache-strategy.md` y la implementación real de `sw.js` (nombres de caché, mención de `/offline.html` inexistente, y `skipWaiting` no implementado), que Germán corrigió en el PR #7 antes de esta entrega.
- Decisión que puedo explicar y por qué: Agregué `happy-dom` como entorno de pruebas en `vitest.config.mts` en lugar de dejarlo en el entorno `node` por defecto, porque mis pruebas de `offline.spec.ts` necesitan simular (mock) el objeto `navigator.serviceWorker`, y sin un entorno de navegador simulado, Vitest no podía cargar esas pruebas (fallaban con "0 passed" sin ningún error explícito).
- Comando o prueba proporcionada que ejecuté: `npm run test -- --run` y `npm run verify` (que ejecuta ambos, más `npm run build`).
- Resultado real que observé: Las 3 suites de prueba pasaron: `starter.spec.mjs` (Semana 1), `manifest.spec.ts` (Semana 2, 12 pruebas), y mis dos suites nuevas (`service-worker.spec.ts` con 17 pruebas, `offline.spec.ts` con 9 pruebas) — 38 pruebas en total. El build de Next.js compiló sin errores.
- Qué verifica esa prueba y qué no verifica: `service-worker.spec.ts` verifica que `sw.js` registra los listeners correctos (`install`, `activate`, `fetch`), que precachea los recursos críticos del shell, y que limpia cachés viejos al activarse — todo leyendo el archivo como texto, no ejecutándolo. `offline.spec.ts` verifica el comportamiento de `registerSW()`/`unregisterSW()` ante navegadores sin soporte, registro exitoso y fallos, usando mocks. Ninguna de las dos ejecuta un Service Worker real interceptando peticiones de red en un navegador.
- Limitación, dificultad o riesgo que identifiqué: Las pruebas basadas en lectura de texto (regex sobre el código fuente) son frágiles ante refactors: si alguien reescribe `sw.js` de forma funcionalmente equivalente pero con otra estructura de código, las pruebas podrían fallar sin que haya un bug real.
- Uso de IA: usé Claude para diseñar la estructura de ambas suites de prueba a partir del código real de `sw.js` y `register-service-worker.ts`, para diagnosticar por qué las pruebas daban "0 passed" (faltaba el entorno `happy-dom`), y para identificar la inconsistencia entre la documentación de Germán y el código de Irvin. Escribí y ejecuté yo misma todos los comandos antes de aceptarla.

---------------------------------------------------------------------------------------------

# Evidencia individual — Semana 4 (w04-csr-ssr)

## Integrante: Germán Yair Martinez Bolaños

- Estudiante: Germán Yair Martinez Bolaños
- Commit SHA evaluado: 4f1946cc95e53eda34ee1cd6365d834d66c8340c
- Mi contribución concreta y enlace a archivo, commit anterior o revisión: Implementé la página de detalle de inspección SSR en `src/app/inspecciones/[id]/page.tsx`, utilizando `generateStaticParams` y `generateMetadata` para pre-renderizar el HTML estático de cada registro sintético. Configuré los estados de carga y error en `src/components/loading-state.tsx` (esqueleto animado, spinner y not-found). Redacté la sección correspondiente al detalle SSR en `docs/rendering-decision.md` explicando los trade-offs de la elección. Añadí pruebas unitarias en `tests/rendering.spec.ts` que validan tanto CSR como SSR. Actualicé el README.md y el workflow de GitHub Actions.
- Decisión técnica que puedo explicar: Decidí que la página de detalle de inspecciones fuese un Server Component (SSR / SSG) porque el estado no necesita cambiar dinámicamente con interacción del usuario en la primera carga, lo que elimina el `flash de carga` (spinner) que ocurre en el cliente y permite entregar el HTML completo desde el inicio, mejorando el TTFB percibido. Opté por usar `generateStaticParams` para pre-generar las rutas de las inspecciones que sabemos existen en la base de datos sintética, y así servir esos archivos estáticamente.
- Prueba que ejecuté y resultado: Ejecuté `npm run test -- --run` tras corregir un problema de dependencia faltante `@testing-library/react` y configurar `vitest.config.mts` para inyectar React usando ESBuild, pasando exitosamente los 41 tests del proyecto, incluyendo los nuevos de `rendering.spec.ts` que verifican el skeleton de CSR y los datos completos en el SSR.
- Limitación o fallo diagnosticado: La renderización estática (SSG) que implementé vía `generateStaticParams` asume que los identificadores de inspección están precargados. Si agregamos una nueva inspección sin recompilar el proyecto ni configurar `fallback`, la aplicación devolvería 404 para ese ID. A futuro necesitaríamos habilitar revalidación (ISR) para manejar datos dinámicos que surjan del lado del cliente.
- Cambio que podría defender o modificar en vivo: El test de rendering usa `React.createElement` en vez de JSX directamente, debido a que se requería el uso obligatorio de `.ts` (en lugar de `.tsx`) en el nombre de archivo de la prueba por la configuración estricta de evaluación, y quise asegurar que la compilación de ESBuild pasara sin problemas. En la vida real, lo cambiaría a `.tsx` para escribir los tests más fluidamente.
- Uso declarado de IA (herramienta, propósito, validación): Utilicé Antigravity (IA) para implementar la base del detalle SSR y los estilos CSS, así como para ajustar el archivo de tests `rendering.spec.ts`. Revisé y modifiqué manualmente los resultados para que se ajustaran al objetivo (por ejemplo, corregir dependencias y refactorizar el código de prueba de Testing Library para saltarse el JSX que lanzaba error).