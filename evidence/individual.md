...
- Qué verifica esa prueba y qué no verifica: `service-worker.spec.ts` verifica que `sw.js` registra los listeners correctos (`install`, `activate`, `fetch`), que precachea los recursos críticos del shell, y que limpia cachés viejos al activarse — todo leyendo el archivo como texto, no ejecutándolo. `offline.spec.ts` verifica el comportamiento de `registerSW()`/`unregisterSW()` ante navegadores sin soporte, registro exitoso y fallos, usando mocks. Ninguna de las dos ejecuta un Service Worker real interceptando peticiones de red en un navegador — esa cobertura requeriría pruebas E2E (por ejemplo con Playwright).
- Limitación, dificultad o riesgo que identifiqué: Las pruebas basadas en lectura de texto (regex sobre el código fuente) son frágiles ante refactors: si alguien reescribe `sw.js` de forma funcionalmente equivalente pero con otra estructura de código, las pruebas podrían fallar sin que haya un bug real. Es un trade-off consciente frente a no tener ninguna cobertura, dada la limitación de Vitest para ejecutar Service Workers reales.
- Uso de IA: usé Claude para diseñar la estructura de ambas suites de prueba a partir del código real de `sw.js` y `register-service-worker.ts` que compartí, para diagnosticar por qué las pruebas daban "0 passed" (faltaba el entorno `happy-dom`), y para identificar la inconsistencia entre la documentación de Germán y el código de Irvin comparando ambos textualmente. Escribí y ejecuté yo misma todos los comandos, y revisé cada prueba generada contra el código real antes de aceptarla.